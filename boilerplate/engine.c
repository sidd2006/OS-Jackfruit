...
/* --- Producer: reads from pipe fd and pushes into bounded buffer --- */
typedef struct {
    int read_fd;
    char container_id[CONTAINER_ID_LEN];
...
static pid_t launch_container(supervisor_ctx_t *ctx, const control_request_t *req)
{
    ...
    if (!stack) { perror("malloc"); close(pipefd[0]); close(pipefd[1]); return -1; }
...
static void handle_client(supervisor_ctx_t *ctx, int client_fd)
{
    ...
    if (req.kind == CMD_START || req.kind == CMD_RUN) {
        ...
        if (fd < 0) {
            ...
            resp.status = 0;
            snprintf(resp.message, CONTROL_MESSAGE_LEN, "Log for %s:", req.container_id);
            send(client_fd, &resp, sizeof(resp), 0);
            char lbuf[4096];
            ssize_t n;
            while ((n = read(fd, lbuf, sizeof(lbuf))) > 0)
        ...
        snprintf(resp.message, CONTROL_MESSAGE_LEN,
                 found ? "Stopped %s" : "Container %s not found", req.container_id);
    } else {
        resp.status = -1;
