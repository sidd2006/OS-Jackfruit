...
static long get_rss_bytes_by_pidstruct(struct pid *pid_struct)
{
...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        rss = get_rss_bytes_by_pidstruct(entry->pid_struct);
        ...
        if ((unsigned long)rss >= entry->hard_limit_bytes) {
            ...
            rcu_read_lock();
            task = pid_task(entry->pid_struct, PIDTYPE_PID);
            if (task)
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (!pid_struct) {
            printk(KERN_WARNING "[container_monitor] PID %d not found\n", req.pid);
            return -ESRCH;
        }
